...
$auth = auth();
if ($auth['level'] == 'supplier' || (isset($_GET['supplier_group']) && !empty($_GET['supplier_group']))) {
    $code = $auth['level'] == 'supplier' ? $auth['code'] : $_GET['supplier_group'];
    $data = array_filter($data, function ($row) use ($code) {
        return $row[8] == $code;
    });
}

unset($_GET['supplier_group']);
